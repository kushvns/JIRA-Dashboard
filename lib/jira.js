...
function buildJql() {   return `     project in ("SE", "IN")     AND assignee in (       "712020:d57549f7-9a84-4aba-8f31-037b455a27a9",       "712020:8d513cb6-82f0-4afb-a2c9-78035336780e",       "712020:c5853dbe-0855-49e0-b65b-0edcb9530e89",       "712020:860bafeb-410d-444b-91cb-51d3a1d38386",       "712020:90df16d8-d633-422c-b8cd-871da176aa67",       "712020:695d871d-ef29-4b2b-bbda-a4a73db7ab5e",       membersOf("IT-OPS"),       membersOf("DC-OPS")     )     AND (       created >= startOfDay(-6d)       OR resolutiondate >= startOfDay(-6d)       OR resolution IS EMPTY     )     ORDER BY updated DESC   `; }
  const people = accounts.map(({ accountId }) => `\"${accountId}\"`).join(", ");
  return `project in ("SE", "IN") AND assignee in (${people}) AND (created >= startOfDay(-6d) OR resolutiondate >= startOfDay(-6d) OR resolution IS EMPTY) ORDER BY updated DESC`;
}

async function searchIssues(slaFields, accounts) {
...
export async function buildDashboardData() {
  ...
  const rawIssues = await searchIssues(slaFields, accounts);
  const created = scoped.filter(   (issue) => issue.createdInWindow );  const resolved = scoped.filter(   (issue) => issue.resolvedInWindow );  const open = scoped.filter(   (issue) => issue.open && issue.createdInWindow );  const breached = resolved.filter(   (issue) => issue.breached );  const completedSla = resolved.filter(   (issue) => issue.completedSla );  const resolutionValues = resolved   .map((issue) => issue.resolutionHours)   .filter(Number.isFinite); rawIssues
  .map((issue) =>
    publicIssue(issue, slaFields, accountsById, start, now)
  )
  ...
  const team = ASSIGNEES.map((email) => {
    ...
    const mineOpen = mine.filter(   (issue) => issue.open && issue.createdInWindow );  const mineBreached = mineResolved.filter(   (issue) => issue.breached ); mine.filter((issue) => issue.resolvedInWindow);
    const mineOpen = mine.filter((issue) => issue.open);
    ...
    const hours = mineResolved   .map((issue) => issue.resolutionHours)   .filter(Number.isFinite);  const project = Object.fromEntries(   PROJECTS.map((key) => {     const rows = mine.filter(       (issue) => issue.project === key     );      return [       key,       {         created: rows.filter(           (issue) => issue.createdInWindow         ).length,         closed: rows.filter(           (issue) => issue.resolvedInWindow         ).length,         open: rows.filter(           (issue) => issue.open && issue.createdInWindow         ).length,       },     ];   }) );  return {   email,   name: mine.find((issue) => issue.assigneeName)?.assigneeName     || email.split("@")[0],   created: mineCreated.length,   closed: mineResolved.length,   open: mineOpen.length,   breached: mineBreached.length, Object.fromEntries(PROJECTS.map((key) => {
