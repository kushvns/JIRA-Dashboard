...
function buildJql(accounts) {
  const people = accounts.map(({ accountId }) => `\"${accountId}\"`).join(", ");
  return `project in (${PROJECTS.join(", ")}) AND assignee in (${people}) AND (created >= startOfDay(-6d) OR resolutiondate >= startOfDay(-6d) OR resolution IS EMPTY) ORDER BY updated DESC`;
}
...
export async function buildDashboardData() {
  ...
  const scoped = rawIssues
    ...
    .filter((issue) => ASSIGNEES.includes(issue.assignee.toLowerCase()));

  const created = scoped.filter((issue) => issue.createdInWindow);
  const resolved = scoped.filter((issue) => issue.resolvedInWindow);
  const open = scoped.filter((issue) => issue.open);
  const breached = scoped.filter((issue) => issue.breached);
  ...
  const team = ASSIGNEES.map((email) => {
    ...
    const mineBreached = mine.filter((issue) => issue.breached);
    ...
    const project = Object.fromEntries(PROJECTS.map((key) => {
      ...
      return [key, {
        ...
        open: rows.filter((issue) => issue.open).length,
      }];
    }));
    return {
      email,
  ...
  return {
    ...
    projectTotals: Object.fromEntries(PROJECTS.map((key) => {
      ...
      return [key, {
        ...
        open: rows.filter((issue) => issue.open).length,
        breached: rows.filter((issue) => issue.breached).length,
      }];
    })),
    team,
    daily: days,
